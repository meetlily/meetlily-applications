...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  if (req.method === "POST") {
    ...
    try {
      ...
      if (currentUser && currentUser?.Role.length > 0) {
        ...
        if (currentUser?.Role[0].name === "Super Administrator") {
          ...
          res
            .status(401)
            .json({ error: "Your do not have permission to view users" })
        }
      }
      res.status(500).json({
        error:
    ...
    const { id, name, slug, description, logo, industry } = body
    const foundSupAdmin = currentUser?.Role.find(
      (obj: { name: string }) => obj.name === "Super Administrator"
    )
    if (foundSupAdmin) {
    ...
    res
      .status(400)
      .json({ error: "Your do not have permission to delete organization" })
  } else {
